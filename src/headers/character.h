...
init_character_frames(int num_columns, int num_rows, texture_data *tex, const texture_atlas *atlas) {
    ...
    for (int i = 0; i < num_frames; i++) {
        ...
        character_frame.left = ((0.0f + x_left_crop + x_offset) * (float) tex->width / (float) atlas->width) +
                               ((float) tex->x_atlas_offset / (float) atlas->width);
        character_frame.right = ((1.0f / 4 - x_right_crop + x_offset) * (float) tex->width / (float) atlas->width) +
                                ((float) tex->x_atlas_offset / (float) atlas->width);
        character_frame.bottom = ((0.0f + y_bottom_crop + y_offset) * (float) tex->height / (float) atlas->height) +
                                 ((float) tex->y_atlas_offset / (float) atlas->height);
        character_frame.top = ((1.0f / 4 - y_top_crop + y_offset) * (float) tex->height / (float) atlas->height) +
                              ((float) tex->y_atlas_offset / (float) atlas->height);
...
unique_ptr<character> initialize_player(const texture_atlas *atlas, std::string file_name) {
    ...
    texture_data tex = image_data_map[file_name];
    p->char_frames = init_character_frames(4, 4, &tex, atlas);
    return p;
}

unique_ptr<character> update_from_user_input(const window_info *win_meta, unique_ptr<character> motion) {
